...
!stop
  ...
  !-----------------------------------------------------------------  
  i=(nsteps+5000)/nstep_avg+1
  allocate(pop(nquant,i),pop_surf(nquant,i),pop_amp(nquant,i))
  allocate(rho(nquant,nquant,i),rho1(i),rho2(i))
...
do i=1,N_traj
    traj_num=i
    V_coup=0.001d0;call setup_parameters
    call init_cond
    call evolve(1,5000)
    V_coup=Vc_save;call setup_parameters
    call evolve(5001,nsteps+5000)
    call average_end
  enddo
call write_average
...
do i=1,nquant
ci(i)=sum(vect1(:,i)*vect0(:,1))
enddo

ci=ci/sqrt(sum(ci**2))

ci=matmul(transpose(si_adiab),ci)
...
do j=1,nquant/2
rho1(:)=rho1(:)+rho(j,j,:)
rho2(:)=rho2(:)+rho(j+nquant/2,j+nquant/2,:)
 enddo


  do i=1,(nsteps+5000)/nstep_avg
    write(100,'(16f15.7)')(i-1)*nstep_avg*dtc*1.d15,real(rho1(i)),real(rho2(i))
  enddo
...
subroutine setup_parameters
  ...
  !omg=omg_B
  !ck(1)=dsqrt(lambda_B*mass(1)*omg_B**2/2.d0)
  ...
  Hamil_site(1,2)=V_coup; Hamil_site(2,1)=Hamil_site(1,2)
...
g_coup=dsqrt(0.5d0*mass(1)*lambda_B)*omg_B
...
H1=sum(0.5d0*mass(1)*((omg*x)**2))
H2=sum(((ck**2)/(2d0*mass(1)*omg**2)))
H3=sum((x*ck))
...
delV_dels=0.d0
  ...
  do i=1,nquant/2
  do j=1,nquant/2
H_diab(i,j)=H_diab(i,j)+(Hamil_site(1,1)+ens1(i))*expec11(i,j)
H_diab(i,j)=H_diab(i,j)+H1*expec11(i,j)
!H_diab(i,j)=H_diab(i,j)+H2*expec_sqr_q11(i,j)
H_diab(i,j)=H_diab(i,j)-H3*expec_q11(i,j)

H_diab(i+nquant/2,j+nquant/2)=H_diab(i+nquant/2,j+nquant/2)+(Hamil_site(2,2)+ens2(i))*expec22(i,j)
H_diab(i+nquant/2,j+nquant/2)=H_diab(i+nquant/2,j+nquant/2)+H1*expec22(i,j)
!H_diab(i+nquant/2,j+nquant/2)=H_diab(i+nquant/2,j+nquant/2)+H2*expec_sqr_q22(i,j)
H_diab(i+nquant/2,j+nquant/2)=H_diab(i+nquant/2,j+nquant/2)-H3*expec_q22(i,j)

  H_diab(i,j+nquant/2)=Hamil_site(1,2)*(expec12(i,j))
  H_diab(i+nquant/2,j)=Hamil_site(2,1)*(expec21(i,j))

  delV_dels(i,j,:)=(mass*(omg**2)*x)*expec11(i,j)-ck*expec_q11(i,j)
  delV_dels(i+nquant/2,j+nquant/2,:)=(mass*(omg**2)*x)*expec22(i,j)-ck*expec_q22(i,j)
  delV_dels(i,j+nquant/2,:)=0d0
  delV_dels(i+nquant/2,j,:)=0d0

  enddo
  enddo

!write(3,*) delV_dels(1,4,:)
  !do i=1,nquant
  !do j=1,nquant
!write(*,*) H_diab(i,j)
!enddo
...
!stop
  ...
  !do i=1,nquant
   ! j=(i-1)*nw
    !H_diab(i,i)=H_diab(i,i)+H1+sum(ck(j+1:j+nw)*x(j+1:j+nw))
    !delV_dels(i,i,:)=grad_H1
    !delV_dels(i,i,j+1:j+nw)=delV_dels(i,i,j+1:j+nw)+ck(j+1:j+nw)
  !enddo
